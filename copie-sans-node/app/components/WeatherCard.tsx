...
function getEmoji(iconCode: string) { return ICON_MAP[iconCode.slice(0, 2)] ?? "🌡️" }
...
export default function WeatherCard() {
    ...
    return (
        <Card title="Météo" emoji="⛅" bgColor="#dce8f4" accent="#2d4a6a">
            {loading && <p style={{ color: "var(--text-muted)", marginTop: "8px" }}>📍 Localisation en cours...</p>}
            {error && <p style={{ color: "var(--text-muted)", marginTop: "8px" }}>⚠️ {error}</p>}
            {weather && (
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                        <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", flex: 1 }}>📍 {weather.city}</p>
                        {[`💨 ${weather.wind} km/h`, `💧 ${weather.humidity}%`].map(t => (
                            <span key={t} style={{ background: "rgba(45,74,106,0.08)", borderRadius: "8px", padding: "3px 8px", fontSize: "0.75rem", color: "var(--text-muted)", whiteSpace: "nowrap" }}>{t}</span>
                        ))}
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                        {[{ label: "Matin", data: weather.morning }, { label: "Après-midi", data: weather.afternoon }].map(({ label, data }) => (
                            <div key={label} style={{ background: "rgba(45,74,106,0.06)", borderRadius: "14px", padding: "12px", textAlign: "center" }}>
                                ...
                                <p style={{ fontSize: "1.6rem", fontWeight: 700, fontFamily: "var(--font-jost)", color: "var(--text-dark)" }}>{data.temp}°</p>
