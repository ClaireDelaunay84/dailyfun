...
// Signe astrologique selon la date
...
export default function EphemeridesPreview({ clickable = false }: { clickable?: boolean }) {
    const [data, setData] = useState<EphemData | null>(null)
    ...
    useEffect(() => {
        ...
        // Import dynamique des fêtes
        import("../../data/fetes").then(({ getFeteduJour }) => {
    ...
    return (
        <div className={`${styles.card} ${clickable ? styles.cardClickable : ""}`}>
            <div className={styles.cardTitle}>ÉPHÉMÉRIDES</div>
            ...
            <div className={styles.ephemRow}>
                ...
                <div className={styles.ephemText}>
                    Jour {data.jourAnnee} de l'année · {data.joursRestants} jours restants
                </div>
            </div>
            <div className={styles.ephemRow}>
                <span className={styles.ephemIcon}>🌙</span>
                <div className={styles.ephemText}>
                    {data.lune} · Lever {data.lever} · Coucher {data.coucher}
                </div>
            </div>
            <div className={styles.ephemRow}>
                <span className={styles.ephemIcon}>⭐</span>
            ...
            {clickable && <div className={styles.cardCta}>Voir tout ›</div>}
        </div>
    )
}